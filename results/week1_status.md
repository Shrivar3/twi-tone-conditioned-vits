...
However, the round-trip ASR/WER result is not valid yet. The ASR step used a Whisper-style model path/configuration, which failed for Twi with "Unsupported language: twi". As a result, the committed WER/CER values are diagnostic failure outputs only and must not be treated as real baseline scores.
...
- The Farmerline Qwen2-Audio ASR model is an 8B BF16 model, so the next GPU instance must have enough VRAM and a compatible Transformers installation.
